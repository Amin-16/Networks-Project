...
class TestOneToOneChat(unittest.TestCase):
    ...
    @patch('peer.getUserByUsername', return_value={"online": True, "address": {"portRecv": 1234}})
    @patch('socket.socket')
    def test_join_one2one_successful(self, mock_socket, mock_getUserByUsername, mock_input):
        chat_peer = {"online": True, "address": {"portRecv": 1234}}
        username = "testuser"
        join_one2one(chat_peer, username)
        mock_socket.assert_called_once_with(socket.AF_INET, socket.SOCK_STREAM)
        mock_socket.connect.assert_called_once_with((socket.gethostname(), 1234))
        mock_socket.send.assert_called_once_with(
            b'{"type": "chat_request", "method": "invite", "username": "testuser"}')
        mock_socket.recv.assert_called_once_with(1024)
        mock_socket.close.assert_called_once()

    @patch('builtins.input', side_effect=["testuser"])
    @patch('peer.getUserByUsername', return_value={"online": False})
    @patch('socket.socket')
    def test_join_one2one_offline_user(self, mock_socket, mock_getUserByUsername, mock_input):
        chat_peer = {"online": False}
        username = "testuser"
        join_one2one(chat_peer, username)
