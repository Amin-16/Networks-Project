...
def format_message(username, message):
    def emphasize_text(match):
        return f'\033[1m{match.group(1)}\033[0m'  # Emphasize text (bold)

    def italicize_text(match):
        return f'\033[3m{match.group(1)}\033[0m'  # Italicize text

    def underline_text(match):
        return f'\033[4m{match.group(1)}\033[0m'  # Underline text

    def crossline_text(match):
        return f'\033[9m{match.group(1)}\033[0m'  # Strikethrough (crossline) text
    ...
    # Underline text
    message = re.sub(r'~([^~]+)~', underline_text, message)

    # Strikethrough (crossline) text
    message = re.sub(r'-(.*?)-', crossline_text, message)
